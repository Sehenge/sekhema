...
class TelegramService
{
    ...
    public function startBot(array $request): void
    {
        ...
        $onboardingText = 'Привет! Меня зовут Алиса. Предлагаю познакомиться и начать общаться, о чём ты хочешь поговорить? Или задать мне вопрос? 🔥';
    ...
    public function sendBalanceInfo(array $request): void
    {
        ...
        if ($balance && $balance->plan_tokens) {
            $balanceText =
                '💼 Основной баланс: '.$balance->plan_tokens." токенов.\n".
                'Израсходовано: '.$balance->used_plan_tokens.' токенов.';

        } else {
            $balanceText =
                '🧪 Пробный период: '.$balance->trial_tokens." токенов. \n".
                'Использовано: '.$balance->used_trial_tokens.' токенов.';
        }
